<template>
    <div class="card card-default card-no-padding">
        <div class="card-header">
            <button class="btn btn-primary float-right" @click="showCreateForm = true; editingMonitor = null;" :disabled="showCreateForm">Create Schedule Monitor</button>
            Schedule Monitors
        </div>
        <div class="card-body">
         ...
         -&gt;monitor();
            ...
            <div v-if="monitors.data && monitors.data.length">
                <table class="table">
                    ...
                    <tbody>
                        <tr v-for="monitor in monitors.data" :key="monitor.id" :class="{ 'editing-hide': (editingMonitor && editingMonitor.id !== monitor.id), 'is-editing': (editingMonitor && editingMonitor.id === monitor.id)  }">
                            <td>
                                <span :class="{ 'text-success': monitor.status == 'passing', 'text-danger': monitor.status == 'failing' }" v-if="monitor.last_run_at">
                                    {{ monitor.status }}
                                </span>
                                <span class="text-muted" v-else>waiting</span>
                            </td>
                            <td>
                                <a :href="'/schedule-monitor/' + monitor.id">
                                    <template v-if="monitor.name">
                                        {{ monitor.name }}<br>
                                        <span class="d-inline-block text-truncate text-muted small" style="max-width: 350px;" :title="monitor.command">{{ monitor.command }}</span>
                                    </template>
                                    <template v-else>
                                        <span class="d-inline-block text-truncate" style="max-width: 350px;" :title="monitor.command">{{ monitor.command }}</span>
                                    </template>
                                </a>
                            </td>
                            <td class="text-center">
                                <span :title="monitor.schedule">{{ humanSchedule(monitor.schedule) }}</span><br>
                ...
                <div class="row">
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <select id="input_timezone" class="form-control"
                            ...
                            :class="{'is-invalid': editForm.errors.has('timezone')}">
                            <option :value="timezone" v-for="timezone in timezones" :key="timezone">{{ timezone }}</option>
                        </select>
                        <span class="invalid-feedback" v-show="editForm.errors.has('timezone')">
                            {{ editForm.errors.get('timezone') }}
                        </span>
                    </div>
                </div>
                <div class="row">
                    <div class="form-group col-md-6">
                        <label for="input_grace_period">Send alerts after (mins)</label>
                        <input type="text" id="input_grace_period" class="form-control" placeholder="3"
                            v-model="editForm.grace_period"
                            :class="{'is-invalid': editForm.errors.has('grace_period')}">
                        <small class="form-text text-muted">
                            If a job doesn't run on schedule we'll send alerts after this many minutes.
                        </small>
                        <span class="invalid-feedback" v-show="editForm.errors.has('grace_period')">
                            {{ editForm.errors.get('grace_period') }}
                        </span>
                    </div>
                </div>
                <div class="row">
                    <div class="form-group col-md-6">
                        ...
                        <input type="text" id="input_command" class="form-control" placeholder="php artisan run-command"
                            ...
                            :class="{'is-invalid': editForm.errors.has('command')}">
                        <small class="form-text text-muted">
                            The command this schedule should run.
                        </small>
                        <span class="invalid-feedback" v-show="editForm.errors.has('command')">
                            {{ editForm.errors.get('command') }}
                        </span>
                    </div>
                    <div class="form-group col-md-6">
                        <label for="input_name">Name (optional)</label>
                        ...
                        <span class="invalid-feedback" v-show="editForm.errors.has('name')">
                            {{ editForm.errors.get('name') }}
                        </span>
                    </div>
                </div>
                <button type="button" class="btn btn-secondary" @click="editingMonitor = null" :disabled="editForm.busy">Cancel</button>
                <button type="submit" class="btn btn-primary" :disabled="editForm.busy">Save</button>
            </form>
        </div>
...
export default {
    ...
    computed: {
        formScheduleHuman() {
            return this.humanSchedule(this.form.schedule);
        },
        editFormScheduleHuman() {
            return this.humanSchedule(this.editForm.schedule);
        },
        timezones() {
    ...
    methods: {
        ...
        create() {
            Spark.post('/api/schedule-monitors', this.form)
                .then(response => {
        ...
        editMonitor(monitor) {
            ...
            this.editForm.resetStatus();
            this.editForm.name         = monitor.name;
            this.editForm.command      = monitor.command;
            this.editForm.schedule     = monitor.schedule;
            this.editForm.timezone     = monitor.timezone;
            this.editForm.grace_period = monitor.grace_period;
        },
        edit() {
            Spark.put('/api/schedule-monitors/' + this.editingMonitor.id, this.editForm)
                .then(response => {
        ...
        deleteMonitor(monitor) {
            if (!confirm('Are you sure you want to delete this schedule monitor?')) {
                return;
            }

            axios.delete(`/api/schedule-monitors/${monitor.id}`)
                .then(() => {
                    this.getScheduleMonitors();
                });
        }
